...
class GeneticMusic:
    ...
    def generate_initial_population(self, size=10, mode=None):
        population = []
        for _ in range(size):
            # Choose a random hexagram
            hexagram = random.choice(self.hexagrams)
            print(hexagram)
            motif = []
    ...
    def generate_next_generation(self, current_generation, mode, generation, max_generations):
        ...
        while len(next_generation) < len(current_generation):
            ...
            child = self.mutate(child, mode, generation, max_generations)
            next_generation.append(child)

        return next_generation

    def run_genetic_algorithm(self, generations, population_size, mode):
        ...
        for generation in range(generations):
            ...
            population = self.generate_next_generation(
                population, mode, generation, generations)
...
def main(generations, population_size):
    ...
    final_motifs = gm.run_genetic_algorithm(
        generations=generations, population_size=population_size, mode=mode)
